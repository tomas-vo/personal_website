...
const rlTranslations = {
  es: {
    ...
    sub_ejemplo: "Así llega un producto desde la web del proveedor (una fila scrapeada en Google Sheets) y así queda publicado en Shopify después de pasar por n8n y Gemini.",
    ...
    sub_arquitectura: "Esta es la foto completa antes de entrar en detalle. Todo corre en un solo servidor: Google Sheets funciona como base de datos intermedia (staging) y como libro de ventas, n8n orquesta cada paso, y Shopify guarda los datos estructurados de cada producto.",
    ...
    loop_text: "Y cada venta en Shopify dispara dos workflows al mismo tiempo: uno hace el chequeo de stock en la web del proveedor, y el otro registra la venta con su desglose de costos en Google Sheets.",
    ...
    sub_workflows: "Ahora, paso a paso: qué hace cada workflow, qué problema resuelve y cómo lo construí.",
    ...
    wf1_pt2: "Pero ese dato crudo todavía no sirve para vender en Chile: el título viene en portugués y no dice cuánto pesa ni para qué auto es. Ahí entra <strong>Gemini</strong>, que devuelve un <strong>JSON estructurado</strong> con el título traducido con terminología chilena, una descripción HTML, el número OEM, las marcas compatibles, el peso estimado, el tamaño y un estado. Antes de llegar a Gemini, un nodo Code aplica <em>mi propio diccionario</em> de unos 100 términos técnicos PT→ES, para que una misma pieza se llame igual en todo el catálogo.",
    ...
    wf1_pt3: "No todo se puede o conviene traer por correo: <em>motores completos, cajas de cambio, piezas usadas o fluidos inflamables</em>. Gemini clasifica cada producto por peso y riesgo, pero <strong>a veces fallaba</strong>: se colaron motores completos con precios de millones. Por eso agregué un <strong>respaldo determinístico</strong>: una lista de palabras prohibidas en portugués que fuerza el rechazo sin depender del criterio de la IA.",
    wf1_pt4: "Con eso, cada producto aprobado recibe su precio. La idea es que cubra <strong>todo lo que cuesta traer la pieza</strong>, así que primero calculo el <em>costo landed</em>: (costo BRL + flete interno) × tipo de cambio + flete internacional volumétrico + provisión aduanera (IVA + arancel sobre el CIF). Sobre ese costo aplico mi margen.",
    ...
    wf2_pt2: "Para evitarlo, un <strong>Shopify Trigger</strong> activa este workflow con cada pedido creado. El workflow separa el pedido por producto (Split Out), lee el metacampo <code>link_brasil</code> y hace un GET a la ficha del proveedor buscando la palabra <em>\"Esgotado\"</em> (agotado). Si la encuentra, pide un token OAuth nuevo, obtiene la variante y <strong>deja el inventario en 0</strong> en Shopify. Un detalle que aprendí en el camino: la búsqueda corta el HTML antes de la sección <em>\"Produtos relacionados\"</em>, porque los productos relacionados agotados daban falsas alarmas.",
    ...
    sub_shopify: "No solo publico productos. Cada ficha queda con datos estructurados que ordenan la tienda sola y alimentan a los otros workflows.",
    h4_etiquetas: "Etiquetas → colecciones automáticas",
    p_etiquetas: "Quería que alguien con un Jeep encontrara rápido las piezas para su auto. Por eso Gemini extrae del título las marcas compatibles como un arreglo (solo las que aparecen, nunca las inventa) y cada marca se guarda como etiqueta. Así, las colecciones inteligentes de Shopify (Jeep, Fiat, RAM…) se llenan solas, y una pieza compatible con varias marcas aparece en todas sus colecciones.",
    ...
    p_variante: "El producto se crea con un HTTP Request directo a la API REST de Shopify, no con el nodo nativo de n8n, porque ese nodo no permitía controlar la variante completa (precio, peso y SKU). Así cada producto queda listo para vender:",
    pv1: "<strong>SKU</strong> = número OEM, que también va en el título para mejorar la búsqueda, porque muchos clientes buscan la pieza por ese número.",
    ...
    p_meta: "Los metacampos son campos extra que Shopify guarda en cada producto, como notas pegadas a la ficha. Cada producto se publica con siete: el link del proveedor y todo el desglose de costos. Definí cada uno en Shopify para que se vean en el panel de administración.",
    ...
    lesson1_p: "Al principio, el workflow mezclaba datos: un producto quedaba con el precio o la imagen de otro. La causa era que referenciaba los datos entre nodos por índice, y cuando un Filter o un If descartaba items, los índices dejaban de coincidir. Lo resolví usando el rastreo de items de n8n (<code>.item</code>), que sigue a cada producto a través de toda la cadena.",
    lesson2_h: "La IA no basta sola",
    lesson2_p: "Gemini clasifica bien casi siempre, pero en e-commerce un solo error cuesta caro, como publicar un motor completo a precio de repuesto. Aprendí a no depender solo de la IA: toda decisión crítica tiene además una regla determinística de respaldo.",
    lesson3_h: "Permisos, tokens y metacampos",
    lesson3_p: "Conectarme a la API de Shopify me enseñó cosas que no salen en los tutoriales: la app necesita scopes específicos (como <code>write_inventory</code>) para modificar el inventario, conviene pedir un token nuevo en cada ejecución para que no expire, hay que agregar pausas entre llamadas para no superar el rate limit, y los metacampos solo aparecen en el panel si antes creas su definición.",
    lesson4_h: "Costo mínimo, control máximo",
    lesson4_p: "Elegí n8n self-hosted en vez de un SaaS que cobra por ejecución. Con un servidor de 5 USD al mes puedo correr flujos ilimitados, y lo único que pago por uso es la API de Gemini.",
  ...
  en: {
    ...
    sub_ejemplo: "This is how a product arrives from the supplier's website (a scraped row in Google Sheets), and this is how it ends up published on Shopify after going through n8n and Gemini.",
    ...
    sub_arquitectura: "This is the full picture before getting into detail. Everything runs on a single server: Google Sheets works as the intermediate database (staging) and as the sales ledger, n8n orchestrates every step, and Shopify stores the structured data for each product.",
    ...
    loop_text: "And every sale on Shopify triggers two workflows at the same time: one checks stock on the supplier's website, and the other logs the sale with its cost breakdown in Google Sheets.",
    ...
    sub_workflows: "Now, step by step: what each workflow does, what problem it solves, and how I built it.",
    ...
    wf2_pt2: "To avoid that, a <strong>Shopify Trigger</strong> fires this workflow on every order created. The workflow splits the order by product (Split Out), reads the <code>link_brasil</code> metafield, and does a GET on the supplier's listing looking for the word <em>\"Esgotado\"</em> (out of stock). If it finds it, it requests a fresh OAuth token, fetches the variant, and <strong>sets the inventory to 0</strong> on Shopify. A detail I learned along the way: the search cuts off the HTML before the <em>\"Produtos relacionados\"</em> section, because out-of-stock related products were causing false alarms.",
    ...
    sub_shopify: "I don't just publish products. Every listing carries structured data that keeps the store organized on its own and feeds the other workflows.",
    h4_etiquetas: "Tags → automatic collections",
    p_etiquetas: "I wanted someone with a Jeep to quickly find the parts for their car. So Gemini extracts the compatible brands from the title as an array (only the ones that appear, never invented), and each brand gets saved as a tag. That way, Shopify's smart collections (Jeep, Fiat, RAM…) fill themselves in, and a part compatible with several brands shows up in all of its collections.",
    ...
    p_variante: "The product is created with an HTTP Request directly to Shopify's REST API, not with n8n's native node, because that node didn't allow full control over the variant (price, weight, and SKU). This is what makes each product ready to sell:",
    pv1: "<strong>SKU</strong> = the OEM number, which also goes in the title to improve search, because many customers look up the part by that number.",
    ...
    p_meta: "Metafields are extra fields Shopify stores on each product, like notes attached to the listing. Every product is published with seven: the supplier's link and the full cost breakdown. I defined each one in Shopify so they show up in the admin panel.",
    ...
    lesson1_p: "At first, the workflow mixed up data: a product would end up with another one's price or image. The cause was that it referenced data between nodes by index, and whenever a Filter or an If dropped items, the indexes stopped matching up. I fixed it by using n8n's item-linking (<code>.item</code>), which follows each product through the entire chain.",
    lesson2_h: "AI alone isn't enough",
    lesson2_p: "Gemini classifies well almost all the time, but in e-commerce a single mistake gets expensive, like publishing a whole engine at the price of a spare part. I learned not to rely on the AI alone: every critical decision also has a deterministic backup rule.",
    lesson3_h: "Permissions, tokens, and metafields",
    lesson3_p: "Connecting to the Shopify API taught me things that don't show up in tutorials: the app needs specific scopes (like <code>write_inventory</code>) to modify inventory, it's worth requesting a fresh token on every run so it doesn't expire, you have to add pauses between calls to avoid exceeding the rate limit, and metafields only show up in the admin panel once you've created their definition beforehand.",
    lesson4_h: "Minimum cost, maximum control",
    lesson4_p: "I chose self-hosted n8n instead of a SaaS that charges per execution. With a $5/month server I can run unlimited flows, and the only thing I pay for by usage is the Gemini API.",
  ...
  pt: {
    ...
    sub_ejemplo: "Assim chega um produto do site do fornecedor (uma linha raspada no Google Sheets), e assim ele fica publicado na Shopify depois de passar pelo n8n e pelo Gemini.",
    ...
    sub_arquitectura: "Esta é a foto completa antes de entrar em detalhes. Tudo roda em um único servidor: o Google Sheets funciona como banco de dados intermediário (staging) e como livro de vendas, o n8n orquestra cada etapa, e a Shopify guarda os dados estruturados de cada produto.",
    ...
    loop_text: "E cada venda na Shopify dispara dois workflows ao mesmo tempo: um faz a checagem de estoque no site do fornecedor, e o outro registra a venda com seu detalhamento de custos no Google Sheets.",
    ...
    sub_workflows: "Agora, passo a passo: o que cada workflow faz, que problema resolve e como eu o construí.",
    ...
    wf1_pt2: "Mas esse dado bruto ainda não serve para vender no Chile: o título vem em português e não diz quanto pesa nem para qual carro é. É aí que entra o <strong>Gemini</strong>, que devolve um <strong>JSON estruturado</strong> com o título traduzido com terminologia chilena, uma descrição em HTML, o número OEM, as marcas compatíveis, o peso estimado, o tamanho e um status. Antes de chegar ao Gemini, um nó Code aplica <em>meu próprio dicionário</em> de cerca de 100 termos técnicos PT→ES, para que uma mesma peça tenha sempre o mesmo nome em todo o catálogo.",
    ...
    wf2_pt2: "Para evitar isso, um <strong>Shopify Trigger</strong> aciona este workflow a cada pedido criado. O workflow separa o pedido por produto (Split Out), lê o metacampo <code>link_brasil</code> e faz um GET na ficha do fornecedor buscando a palavra <em>\"Esgotado\"</em>. Se encontra, pede um token OAuth novo, obtém a variante e <strong>zera o estoque</strong> na Shopify. Um detalhe que aprendi no caminho: a busca corta o HTML antes da seção <em>\"Produtos relacionados\"</em>, porque produtos relacionados esgotados geravam alarmes falsos.",
    ...
    sub_shopify: "Não só publico produtos. Cada ficha fica com dados estruturados que organizam a loja sozinha e alimentam os outros workflows.",
    h4_etiquetas: "Tags → coleções automáticas",
    p_etiquetas: "Eu queria que alguém com um Jeep encontrasse rápido as peças para o carro dele. Por isso o Gemini extrai do título as marcas compatíveis como um array (só as que aparecem, nunca as inventa), e cada marca é salva como tag. Assim, as coleções inteligentes da Shopify (Jeep, Fiat, RAM…) se preenchem sozinhas, e uma peça compatível com várias marcas aparece em todas as suas coleções.",
    ...
    p_variante: "O produto é criado com um HTTP Request direto para a API REST da Shopify, não com o nó nativo do n8n, porque esse nó não permitia controlar a variante por completo (preço, peso e SKU). Assim cada produto fica pronto para vender:",
    pv1: "<strong>SKU</strong> = número OEM, que também vai no título para melhorar a busca, porque muitos clientes procuram a peça por esse número.",
    ...
    p_meta: "Os metacampos são campos extras que a Shopify guarda em cada produto, como notas coladas na ficha. Cada produto é publicado com sete: o link do fornecedor e todo o detalhamento de custos. Defini cada um na Shopify para que apareçam no painel de administração.",
    ...
    lesson1_p: "No início, o workflow misturava dados: um produto ficava com o preço ou a imagem de outro. A causa era que ele referenciava os dados entre nós por índice, e quando um Filter ou um If descartava items, os índices deixavam de coincidir. Resolvi usando o rastreamento de items do n8n (<code>.item</code>), que segue cada produto por toda a cadeia.",
    lesson2_h: "A IA sozinha não basta",
    lesson2_p: "O Gemini classifica bem quase sempre, mas em e-commerce um único erro sai caro, como publicar um motor completo a preço de peça avulsa. Aprendi a não depender só da IA: toda decisão crítica também tem uma regra determinística de reforço.",
    lesson3_h: "Permissões, tokens e metacampos",
    lesson3_p: "Conectar-me à API da Shopify me ensinou coisas que não aparecem nos tutoriais: o app precisa de scopes específicos (como <code>write_inventory</code>) para modificar o estoque, vale a pena pedir um token novo a cada execução para que não expire, é preciso adicionar pausas entre chamadas para não superar o rate limit, e os metacampos só aparecem no painel se antes você criar sua definição.",
    lesson4_h: "Custo mínimo, controle máximo",
    lesson4_p: "Escolhi n8n self-hosted em vez de um SaaS que cobra por execução. Com um servidor de 5 USD por mês posso rodar fluxos ilimitados, e a única coisa que pago por uso é a API do Gemini.",
  ...
  fr: {
    ...
    sub_ejemplo: "Voici comment un produit arrive depuis le site du fournisseur (une ligne scrapée dans Google Sheets), et comment il finit publié sur Shopify après être passé par n8n et Gemini.",
    ...
    h2_arquitectura: "Architecture",
    sub_arquitectura: "Voici la vue d'ensemble avant d'entrer dans le détail. Tout tourne sur un seul serveur : Google Sheets sert de base de données intermédiaire (staging) et de livre de ventes, n8n orchestre chaque étape, et Shopify stocke les données structurées de chaque produit.",
    ...
    loop_text: "Et chaque vente sur Shopify déclenche deux workflows en même temps : l'un vérifie le stock sur le site du fournisseur, et l'autre enregistre la vente avec son détail de coûts dans Google Sheets.",
    ...
    sub_workflows: "Maintenant, étape par étape : ce que fait chaque workflow, quel problème il résout et comment je l'ai construit.",
    ...
    wf1_pt2: "Mais cette donnée brute ne suffit pas encore à vendre au Chili : le titre arrive en portugais et ne précise ni le poids ni pour quelle voiture c'est. C'est là qu'intervient <strong>Gemini</strong>, qui renvoie un <strong>JSON structuré</strong> avec le titre traduit avec une terminologie chilienne, une description HTML, le numéro OEM, les marques compatibles, le poids estimé, la taille et un statut. Avant d'arriver à Gemini, un nœud Code applique <em>mon propre dictionnaire</em> d'environ 100 termes techniques PT→ES, pour qu'une même pièce porte toujours le même nom dans tout le catalogue.",
    ...
    p_variante: "Le produit est créé avec une HTTP Request directement vers l'API REST de Shopify, et non avec le nœud natif de n8n, car ce nœud ne permettait pas de contrôler la variante dans son intégralité (prix, poids et SKU). Voici ce qui rend chaque produit prêt à vendre :",
    pv1: "<strong>SKU</strong> = le numéro OEM, qui figure aussi dans le titre pour améliorer la recherche, car de nombreux clients recherchent la pièce par ce numéro.",
    ...
    lesson1_p: "Au début, le workflow mélangeait les données : un produit se retrouvait avec le prix ou l'image d'un autre. La cause était qu'il référençait les données entre les nœuds par index, et quand un Filter ou un If écartait des items, les index cessaient de correspondre. J'ai résolu ça en utilisant le suivi d'items de n8n (<code>.item</code>), qui suit chaque produit tout au long de la chaîne.",
    lesson2_h: "L'IA seule ne suffit pas",
    lesson2_p: "Gemini classe bien presque tout le temps, mais dans l'e-commerce, une seule erreur coûte cher, comme publier un moteur complet au prix d'une pièce détachée. J'ai appris à ne pas dépendre uniquement de l'IA : chaque décision critique s'appuie aussi sur une règle déterministe de secours.",
    lesson3_h: "Permissions, tokens et métachamps",
    lesson3_p: "Me connecter à l'API Shopify m'a appris des choses qu'on ne trouve pas dans les tutoriels : l'application a besoin de scopes spécifiques (comme <code>write_inventory</code>) pour modifier le stock, il vaut mieux demander un nouveau token à chaque exécution pour qu'il n'expire pas, il faut ajouter des pauses entre les appels pour ne pas dépasser le rate limit, et les métachamps n'apparaissent dans le panneau que si vous avez d'abord créé leur définition.",
    lesson4_h: "Coût minimal, contrôle maximal",
    lesson4_p: "J'ai choisi n8n auto-hébergé plutôt qu'un SaaS facturé à l'exécution. Avec un serveur à 5 USD par mois, je peux faire tourner des flux illimités, et la seule chose que je paie à l'usage est l'API Gemini.",
